...
export async function POST(request) {
  try {
    ...
    // Send via batch API — one API call per 100 emails, avoids rate limiting
    for (let i = 0; i < emails.length; i += RESEND_BATCH_LIMIT) {
      const chunk      = emails.slice(i, i + RESEND_BATCH_LIMIT)
      const chunkRegs  = regs.slice(i, i + RESEND_BATCH_LIMIT)
    ...
    return Response.json({ success: true, sent, total: regs.length })
  } catch (err) {
