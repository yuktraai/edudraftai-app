/**
 * Phase 11A — Pinecone client
 *
 * One index (edudraftai-rag), one namespace per subject_id.
 * Namespace isolation prevents cross-subject retrieval bleed.
 *
 * Env vars required:
 *   PINECONE_API_KEY
...
export async function upsertChunks(subjectId, chunks) {
  ...
  // Pinecone upsert accepts max 100 vectors per call
  const BATCH = 100
