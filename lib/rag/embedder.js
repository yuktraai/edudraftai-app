 ...
 *
 * Phase 51: meta now accepts { subjectCode, docId, title } for canonical indexing.
 * Legacy fallback: if subjectCode absent but subjectId present, uses subjectId in metadata.
 *
 * @param {string[]} texts
 * @param {{ subjectCode?: string, subjectId?: string, docId: string, title: string }} meta
...
export async function embedChunks(texts, meta) {
  ...
  for (let b = 0; b < texts.length; b += BATCH_SIZE) {
    ...
    for (let i = 0; i < batch.length; i++) {
      // 51.2 — canonical metadata: subjectCode takes priority over legacy subjectId
      const vectorMeta = meta.subjectCode
        ? {
