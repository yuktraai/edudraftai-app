...
// ─── Question Bank / Exam Paper parser (Phase 36) ───────────────────────────
// Handles the new SCTE&VT format:
//   - Exam header block lines (Subject:, Semester:, Full Marks:, etc.)
//   - SECTION A / SECTION B / GROUP A / GROUP B headers (ALL CAPS)
//   - Q1. ... [X Marks] question lines
//   - MCQ option lines: A) / B) / C) / D)
...
function parseQuestionBank(text) {
  ...
  for (const rawLine of lines) {
    ...
    // ── Exam header key-value lines (Subject:, Semester:, Full Marks:, etc.)
    if (/^(Subject|Subject Code|Semester|Full Marks|Time|Time Allowed)\s*:/.test(line)) {
      ...
      paras.push(new Paragraph({
        spacing: { after: 30 },
        children: [
          new TextRun({ text: key + ': ', bold: true, size: 22, font: 'Calibri', color: NAVY }),
          new TextRun({ text: val, size: 22, font: 'Calibri' }),
        ],
      }))
    }
    // ── Instructions block header
    else if (/^Instructions(\s+to\s+Candidates)?\s*:?$/i.test(line)) {
      paras.push(new Paragraph({
        spacing: { before: 100, after: 30 },
        children: [new TextRun({ text: line, bold: true, size: 22, font: 'Calibri', color: NAVY })],
      }))
    }
    // ── Numbered instruction lines: (1) ... (2) ...
    else if (/^\(\d+\)/.test(line)) {
      paras.push(new Paragraph({
        indent:  { left: 200 },
        spacing: { after: 20 },
        children: [new TextRun({ text: plainText(line), size: 20, font: 'Calibri', color: MUTED })],
      }))
    }
    // ── SECTION / GROUP headings (ALL CAPS)
    else if (/^(SECTION|GROUP)\s+[A-C]/i.test(line) || /^#{1,3}\s/.test(line)) {
      ...
      paras.push(new Paragraph({
        ...
        children: [new TextRun({
          text: plainText(text), bold: true, size: 24, color: NAVY, font: 'Calibri',
          allCaps: true,
        })],
      }))
    }
    // ── "(Attempt ANY FIVE…)" or "(Answer ALL…)" sub-instruction
    else if (/^\(Attempt|^\(Answer/.test(line)) {
      paras.push(new Paragraph({
        spacing: { after: 60 },
        children: [new TextRun({ text: line, size: 20, italics: true, font: 'Calibri', color: MUTED })],
      }))
    }
    // ── Q-numbered question: Q1. text [X Marks]
    else if (/^Q\d+[\.\)]\s/.test(line)) {
      // Separate marks tag from question text
      const marksMatch = line.match(/(\[\d+\s*Marks?\])$/i)
      const marksTag   = marksMatch ? marksMatch[1] : null
      const questionText = marksTag ? line.slice(0, line.lastIndexOf(marksTag)).trim() : line

      paras.push(new Paragraph({
        spacing: { before: 120, after: 40 },
        children: [
          ...inlineRuns(questionText, { size: 22, font: 'Calibri' }),
          ...(marksTag ? [
            new TextRun({ text: '  ' + marksTag, bold: true, size: 20, color: TEAL, font: 'Calibri' }),
          ] : []),
        ],
      }))
    }
    // ── Simple numbered question: 1. text [X Marks]
    ...
    // ── End-of-paper marker
    else if (/^\*\*\*\s*End of/.test(line)) {
      paras.push(new Paragraph({
        alignment: AlignmentType.CENTER,
        spacing:   { before: 240, after: 40 },
        children: [new TextRun({
          text: plainText(line), bold: true, italics: true,
