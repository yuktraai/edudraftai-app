 ...
 * unit → topic → subtopics hierarchy from raw SCTEVT syllabus text.
 */
export function buildSyllabusExtractionPrompt(rawText) {
  const systemPrompt = `You are an SCTEVT (State Council for Technical Education and Vocational Training, Odisha) syllabus extraction engine. Your ONLY task is to extract the structured content from raw diploma syllabus PDF text and return it as valid JSON.
