...
const PATTERNS = {
  ...
  // Mid-semester pattern: 50 marks / 1.5 hrs
  //   Q.1 — Answer all 5 sub-parts (a–e), 2 marks each = 10
  //   Q.2 — Answer any 4 of 5 sub-parts (a–e), 5 marks each = 20
  //   Q.3–Q.5 — Long/Numerical, attempt any 2 of 3 × 10 = 20
  //   Total: 10 + 20 + 20 = 50  ✓
  //
  //   masterInstruction count = Q1(1) + Q2(1) + longAttempt(2) = 4 → "any four"
  mid_semester: {
    label:           'Mid-Semester (50 Marks)',
    total:           50,
    duration:        '1½ Hrs',
    duration_mins:   90,
    masterInstruction: 'Answer any four Questions including Q No.1 & 2',
    q1: {
      instruction:  'Answer All questions',
      count:        5,           // sub-parts a through e
      marks:        2,
      total:        10,
    ...
    q2: {
      ...
      countGiven:   5,           // sub-parts a through e
      countAttempt: 4,
      marks:        5,
      total:        20,
    ...
    longQ: {
      count:        3,           // Q.3, Q.4, Q.5
      marks:        10,
      attempt:      2,           // student answers any 2
      total:        20,
      hint:         'Long answer / Numerical with full steps / Neat sketch',
    },
  },
}
...
export function buildExamPaperPrompt({
  ...
  const difficultyContract = DIFFICULTY_CONTRACTS['exam_paper']?.[difficulty] ?? ''

  // Sub-part letter sequences
  const q1Letters  = 'abcdefghij'.slice(0, p.q1.count).split('')
  const q2Letters  = 'abcdefghij'.slice(0, p.q2.countGiven).split('')

  // Long question range: Q.3 through Q.(2+longQ.count)
  const longQStart = 3
  const longQEnd   = 2 + p.longQ.count   // end_sem → 7, mid_sem → 5
  const longQRange = longQStart === longQEnd ? `Q.${longQStart}` : `Q.${longQStart}–Q.${longQEnd}`
  const longQLines = Array.from({ length: p.longQ.count }, (_, i) =>
    `${longQStart + i}     [Long answer / Numerical question ${i + 1}${i === 2 ? ' — with neat sketch or full steps' : ''}]           ${p.longQ.marks}`
  ).join('\n\n')
...
6.  ${longQRange} are STANDALONE questions — ONE question each, NO sub-parts. Generate EXACTLY ${p.longQ.count} long questions.
7.  MARKS ON RIGHT MARGIN — after wide whitespace, NOT in brackets inline.
    ...
    ${longQRange} marks notation: "${p.longQ.marks}"
...
10. ${longQRange}: one or two line stems. Include "with neat sketch" or "with steps" where appropriate.
11. Marks total MUST add up (verify before output):
    ...
    ${longQRange} (attempt ${p.longQ.attempt}): ${p.longQ.attempt} × ${p.longQ.marks} = ${p.longQ.total}
    TOTAL: ${p.q1.total} + ${p.q2.total} + ${p.longQ.total} = ${p.total} ✓
...
16. ${longQRange} must cover DIFFERENT topics — no two long questions on the same subtopic.
17. Do NOT add any explanation or preamble before the paper header.
18. Use "1." and "2." (with period) for Q1 and Q2. Use plain numbers (no period) for long questions — exactly as in official SCTE&VT papers.
...
${longQRange} Outline Answers (Long — subheadings, steps, formulae):
${Array.from({ length: p.longQ.count }, (_, i) => `Q.${longQStart + i}: [detailed outline]`).join('\n')}
--- END ANSWER KEY ---`,
    },
    {
      role: 'user',
